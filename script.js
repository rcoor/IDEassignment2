...
function temperatureChange() {
    ...
    d3.tsv("data.tsv", function (d) {
        ...
        function (error, data) {
            var first50 = data.slice(0, 50);
            var last = data.slice(0, data.length - 4).slice(50, -1);
            var meanFirst50 = meanOfElementInObjects(first50);
...
function temperatureChange2() {
    // set the dimensions and margins of the graph

    var svg = d3.select(".multiLine2 svg"),
        margin = { top: 20, right: 50, bottom: 100, left: 50 },
        width = +svg.attr("width") - margin.left - margin.right,
        height = +svg.attr("height") - margin.top - margin.bottom,
        g = svg.append("g").attr("transform", "translate(" + margin.left + "," + margin.top + ")");
    ...
    d3.tsv("data.tsv", function (d) {
        ...
        function (error, data) {
            ...
            // Set domain of x and y
            x.domain(d3.extent(new Array(12), function (d, i) { return i; }));
            y.domain(d3.extent([-6, 6], function (d) { return d; }));
            ...
            // Create color scale scheme
            var color = d3.scaleLinear().domain([1, last.length])
                .interpolate(d3.interpolateHcl)
                .range([d3.rgb("#dbe2dc"), d3.rgb('#294c5f')]);

            // Plot each line by using callback filtered data. Each callback represents one line.
            createLines(last, function (newData, i) {
                g.append("path")
                    .datum(newData)
                    .attr("class", "line")
                    .attr("d", line)
            ...
            function createLines(data, callback) {
                ...
                data.forEach(function (year) {
                    ...
                    months.forEach(function (month, i) {
                        newData.push({
                            "month": i,
                            "temp": parseFloat(year[month]) - meanFirst50[month]
                        });
                    });
...
function meanOfElementInObjects(inArray) {
    var sum = 0;
    var element = inArray.slice(0, 1)[0];
    months = Object.keys(element).splice(1, 12);
    inArray.forEach((object, i) => {
        element.YEAR = 0;
        months.forEach((month) => {
            if (i === 0) {
                element[month] = 0;
            };
            element[month] += object[month] / inArray.length;
        });
    });
    return element;
}
